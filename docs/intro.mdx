...
Each time a key is updated, you will notice an incrementing **[revision number](distributed-keyvalue-store/revisions)** (e.g., r0, r1, etc.). This value, known as the revision, is a monotonic version number that tracks when a key was last modified. Every time a key is updated or deleted, its revision increments, ensuring strong consistency and strict ordering.
...
The **[Compare-And-Swap (CAS)](distributed-keyvalue-store/cas)** operation ensures atomic updates and prevents race conditions in environments where multiple clients may attempt to modify the same key simultaneously.
