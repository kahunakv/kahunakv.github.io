...
For a local standalone server, clone the Kahuna repository and run:
...
The standalone script starts one node on `http://127.0.0.1:8081` and `https://127.0.0.1:8082` using persistent RocksDB storage under `/tmp/kahuna-standalone`.
