...
Before you proceed, make sure that Kahuna is running on your system. Refer to the **[Server Installation](/docs/server-installation/)** section for instructions.
You will also need the **[Kahuna CLI](/docs/kahuna-cli/)** to execute commands on the server.

For a local standalone server with Docker, run:

```bash
docker run --rm -p 8081:8081 -p 8082:8082 -v kahuna-data:/data --name kahuna kahunakv/kahuna:latest
```

The container exposes `http://127.0.0.1:8081` and `https://127.0.0.1:8082`. The named Docker volume `kahuna-data` keeps the server data across container restarts.

Alternatively, clone the Kahuna repository and run the standalone script:
...
The standalone script starts one node on `http://127.0.0.1:8081` and `https://127.0.0.1:8082` using persistent RocksDB data under `/tmp/kahuna-standalone/data` and a WAL under `/tmp/kahuna-standalone/wal`.
...
To test replication and failover with three local nodes, run the native cluster script instead:

```bash
./scripts/run-cluster.sh
```

Connect the CLI to the complete cluster:
...
The `set` command stores key/value pairs durably in the cluster. Internally, the system routes the request to the leader
node of the corresponding Raft group, which coordinates the operation, achieves consensus, and ensures durable storage and replication. Most key spaces use hash routing by default, while advanced ordered key spaces can opt into key-range routing. The `get`
...
By default, Kahuna ensures strong durability by replicating persistent key/value writes through the key's Raft partition. The client is notified of a successful operation only after the mutation has reached Raft commit.
...
kahuna-cli> let limit = get max_rate_limit
       ...
       ...> set rate_limit_user1 new_current_rate ex 60000
       ...>
...
coordinator records the confirmed reads and writes, then applies the changes across all participating nodes during commit. If an error occurs, the transaction aborts and no changes are applied:
