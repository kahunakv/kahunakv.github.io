...
The container exposes `http://127.0.0.1:8081` for REST, `https://127.0.0.1:8082` for HTTPS REST/gRPC, and `http://127.0.0.1:8083` for cleartext gRPC. The named Docker volume `kahuna-data` keeps the server data across container restarts.

You can also install the server as a .NET global tool:
...
Connect to Docker over HTTPS:

```bash
kahuna-cli -c "https://127.0.0.1:8082" --insecure
```

Connect to Docker or the NuGet tool server over cleartext gRPC:
